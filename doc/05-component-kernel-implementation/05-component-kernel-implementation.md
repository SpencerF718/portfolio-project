...
I'm going to represent my component with 2 arrays.  One array will hold the
custom Game object that I made, and the other will hold the playtime.  This
allows playtimes to be easily paired with a Game, and also let's us set some
fixed limits in terms of the array size.  The convention will indicate that
the array size must be under some size, and that no duplicate games should
appear in the libary.  The correspondence will show how the paired arrays
will be represented.
