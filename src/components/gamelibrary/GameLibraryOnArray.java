...
public class GameLibraryOnArray extends GameLibrarySecondary {
    ...
    @Override
    public GameLibraryOnArray newInstance() {
        return new GameLibraryOnArray();
    }

    @Override
    public void transferFrom(GameLibraryOnArray source) {
        this.librarySize = source.librarySize;
        for (int i = 0; i < this.librarySize; i++) {
            this.gameArr[i] = source.gameArr[i];
            this.playtimeArr[i] = source.playtimeArr[i];
        }
        source.clear();
    }
}
